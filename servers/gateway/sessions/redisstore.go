...
// SetTries sets the time the user has tried to log in. This will continually reset
// if the user keeps trying.
func (rs *RedisStore) SetTries(email string) (int64, error) {
	triesKey := "TRIES_" + email
	pipe := rs.Client.Pipeline()
	incr := pipe.Incr(triesKey)
	pipe.Expire(triesKey, 10*time.Minute)

	_, err := pipe.Exec()
	if err != nil {
		return 0, errors.New("Unable to connect to database")
	}
